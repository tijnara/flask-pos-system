# app.py
# Description: The main Flask application file, configured for PythonAnywhere.
...
# Load Secret Key from Environment Variable (Recommended for Security)
# On PythonAnywhere, set this environment variable via the "Web" tab -> "Environment variables" section.
...
if app.secret_key == "default_secret_key_for_dev_only" and os.environ.get("FLASK_ENV") != "development":
    # Log a warning if using the default key in a non-development environment
    app.logger.warning("SECURITY WARNING: Using default SECRET_KEY. Set the SECRET_KEY environment variable.")

# Load API Key(s) from Environment Variable (Recommended)
# Example: VALID_API_KEYS=key1,key2,key3 (comma-separated)
...
if not VALID_API_KEYS or "YOUR_SUPER_SECRET_API_KEY_12345" in VALID_API_KEYS:
    app.logger.warning("SECURITY WARNING: Using default or no API keys. Set the VALID_API_KEYS environment variable.")
...
# PythonAnywhere captures stdout/stderr for server/error logs.
...
# --- Product Routes ---
# (Keep existing product routes: list_products, add_product, edit_product, delete_product)
@app.route('/products')
def list_products():
...
def pos_interface():
            ...
            if not current_sale["items"]:
                ...
                if sale_id:
                    ...
                    if all_items_added:
                        ...
                        flash(f"Sale #{sale_id} created, but item saving failed.", "error"); return redirect(
                            url_for('pos_interface'))
                else:
    ...
    return render_template('pos/interface.html',
                           title="New Sale", prioritized_products=prioritized_products,
                           other_products=other_products, customers=customers,
                           current_sale_items=current_sale["items"], current_customer=current_sale["customer_name"],
...
# The if __name__ == '__main__': block is removed as PythonAnywhere uses the WSGI file to run the app.
# Keep the print statements below if you want to see them during local testing.
# if __name__ == '__main__':
#     print("-" * 68); print("POS System Starting...")
#     print(f"DB: '{DATABASE_FILE}'. Run 'python database_setup.py' if needed.")
#     print(f"Backups: '{os.path.abspath(BACKUP_DIR)}'.")
#     print(f"URL: http://127.0.0.1:5000"); print("-" * 68)
